...
;(function () {
	...
	#:
		#[0]: var firstChildOfClass = j.el(".class", 1)
		#[2]: var _class = j.el(".class")
		#[3]: var _id = j.el("#id")
		#[4]: var _divs = j.el("div")
		#[5]: var _doc = j.el() | var _doc = document.documentElement
		
	@:
		@(0): <string> "<selector>: id, class or tag selector string"
	...
	j.el = function (sel, single) {
		...
		switch (sel[0]) {
			case "#": // id selector
				return (el = document.getElementById(sel.slice(1))) && el.nodeName ? el : void'';
			case ".": // class selector
				el = document.getElementsByClassName(sel.slice(1)); break;
			default: // tag selector
				el = document.getElementsByTagName(sel);
		}
		return el && el.length ? single ? el[0] : el : void''; // class|tag selector
	};
	
	/*: j.all()
	i: "Invoke a function for each element of a <array>|<nodeList>|<array>."
	#:
		#[0]: j.all( ["#id", .class, j.el("#id"), j.el(".class"), j.el("tag"), [j.el("#foo"), j.el("#qux")], document.getElementByClassName("class"), document.body, window], function (el, i, a, b, c) {...}, [1, 2, 3] )
	...
	#:
		#[0]: var dur = 1000;
			j.animate(
				...
				function (t) { if (t >= dur) { callback(); return true; } }
			})
		#[1]: var dur = 1000;
			j.animate(
				function (t) {
	...
	@:
		...
		@(1): stop: <integer>|<function> stop(t) "duration or function that stops the animation when it returns true"
